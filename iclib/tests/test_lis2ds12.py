...
class LIS2DS12TestCase(TestCase):
    ...
    def test_init_configure(self) -> None:
        ...
        dev.init(OutputDataRate.ODR_100_HZ, FullScale.FS_4G, discard_samples=1)

        # Expected CTRL1 value
        expected_ctrl1 = 0
        expected_ctrl1 |= (OutputDataRate.ODR_100_HZ.value << CTRL1_ODR_SHIFT) & CTRL1_ODR_MASK
        expected_ctrl1 |= (FullScale.FS_4G.value << CTRL1_FS_SHIFT) & CTRL1_FS_MASK
        expected_ctrl1 |= CTRL1_BDU_MASK

        # Find a transfer call that writes to CTRL1 and verify byte value
        found = False
        for args, _kwargs in [c for c in self.mock_i2c.transfer.call_args_list]:
            addr, msgs = args
            if addr != dev.address or not msgs:
                continue
            msg = msgs[0]
            if hasattr(msg, 'data') and len(msg.data) >= 2 and msg.data[0] == Register.CTRL1:
    ...
    def test_configure_hf_odr(self) -> None:
        dev = LIS2DS12(self.mock_i2c)
        dev.init(OutputDataRate.ODR_1600_HZ, FullScale.FS_2G, discard_samples=0)
        # Expect HF bit set in CTRL1
        expected_ctrl1 = 0
        expected_ctrl1 |= (OutputDataRate.ODR_1600_HZ.value << CTRL1_ODR_SHIFT) & CTRL1_ODR_MASK
        expected_ctrl1 |= (FullScale.FS_2G.value << CTRL1_FS_SHIFT) & CTRL1_FS_MASK
