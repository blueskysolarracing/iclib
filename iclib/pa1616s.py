...
class GPOutputSentence(ABC):
    ...
    _STATIC_FIELD_SCHEMA: ClassVar[list]

    @classmethod
    def from_nmea_sentence(cls, nmea_sentence: str) -> Optional[GPOutputSentence]:
        """Create a GPOutputSentence object from an NMEA sentence string. Returns None if the sentence is not parsable.

        :param nmea_sentence: The NMEA sentence string to parse.
        :return: (Optional) The GPOutputSentence object created from the NMEA sentence.
        ...
        if split_NMEA_sentence[0] != cls.MESSAGE_ID:
            raise ValueError(
                f"Invalid NMEA sentence. Expected {cls.MESSAGE_ID} but received {split_NMEA_sentence[0]}"
            )
        ...
        if given_checksum != calculated_checksum:
            raise ValueError(
                f"Invalid checksum. Expected {given_checksum} but received {calculated_checksum}"
            )
        ...
        for i, item in enumerate(cls._STATIC_FIELD_SCHEMA):
            ...
            setattr(
                new_output_sentence_obj, field_name, field_type(raw_field_value_str)
            )
...
class GPLocationDataSentence(GPOutputSentence, ABC):
    ...
    _latitude: Optional[str] = None
    _NS_indicator: Optional[str] = None
    _longitude: Optional[str] = None
    _EW_indicator: Optional[str] = None

    @property
    def latitude(self) -> tuple[int, float, str]:
        """Latitude in (degrees, minutes, direction)"""
        return int(self._latitude[:2]), float(self._latitude[2:]), self._NS_indicator

    @property
    def longitude(self) -> tuple[int, float, str]:
        """Longitude in (degrees, minutes, direction)"""
        return int(self._longitude[:3]), float(self._longitude[3:]), self._EW_indicator


class GPMode(Enum):
    """Mode data transmitted by PA1616S in RMC and VTG mode. See table 8 on datasheet."""
...
class GPGGA(GPLocationDataSentence):
    """Store field values that can be outputted by the GPS for $GPGGA update messages by
    the PA1616S module: Time, position, and fix type data. See table 2 on datasheet.
    """

    class PositionFixIndicator(Enum):
        """Position fix indicator data transmitted by PA1616S in GGA mode. See table 3 on datasheet."""
    ...
    position_fix_indicator: Optional[PositionFixIndicator] = None
    """Indicates the status of the position fix."""
    satelites_used: Optional[int] = None
    ...
    diff_corr_age: Optional[float] = None
    """Age of differential corrections in seconds. ``None`` when DGPS is not used."""


@dataclass
class GPRMC(GPLocationDataSentence):
    """Store field values that can be outputted by the GPS for $GPRMC update messages by
        the PA1616S module: Time, date, position, course and speed data; recommended Minimum Navigation
    Information. See table 8 on datasheet.
    """

    class Status(Enum):
        """Status data transmitted by PA1616S in RMC mode. See table 8 on datasheet."""
    ...
    gp_mode: Optional[GPMode] = None
    """Mode of operation"""


@dataclass
class GPVTG(GPOutputSentence):
    """Store field values that can be outputted by the GPS for $GPVTG update messages by
    the PA1616S module: course and speed information relative to the ground. See Table 9 on datasheet.
    """
    ...
    true_course: Optional[float] = None
    ...
    """Speed over ground in knots"""
    speed_over_ground_kph: Optional[float] = None
...
class GPOutputSentenceOptions(tuple[type[GPOutputSentence], int], Enum):
    """Types of update data packets to be outputted by the GPS PA1616S module. See Table 1 on datasheet."""

    RMC = GPRMC, 1
    """Time, date, position, course and speed data. Recommended Minimum Navigation
    Information.
    """
...
class PA1616S:
    ...
    _message_id_to_outputsentence_type : dict[str, type[GPOutputSentence]] = field(default_factory=lambda: {})

    def __post_init__(self) -> None:

        if self.uart_interface.baudrate != self.BAUD_RATE:
            raise ValueError(
                f"Invalid baud rate. Expected {self.BAUD_RATE} but received {self.uart_interface.baudrate}"
            )
    ...
    def enabled_update_modes(self) -> list[GPOutputSentenceOptions]:
        """The type of update data packet outputted by the GPS module, in the order that they are outputted when receiving an update."""

        return self._enabled_update_modes

    @enabled_update_modes.setter
    def enabled_update_modes(self, modes: list[GPOutputSentenceOptions]) -> None:
        """Set the type of update data packets outputted by the GPS module, in the order that they are outputted when receiving an update."""
    ...
    def update_rate(self) -> int:
        """The rate at which the GPS module sends update packets in milliseconds."""

        return self._update_rate

    @update_rate.setter
    def update_rate(self, rate: int) -> None:
        """Set the rate at which the GPS module sends update packets in milliseconds. See page 8-9 on PMTK datasheet."""

        self._update_rate = rate
        self._send_uart_command(f"PMTK220,{self.update_rate}".encode("ascii"))

    def get_latest_update_packets(self) -> list[GPOutputSentence]:
        """Gets the latest set of update packets from the GPS module as GPOutputSentence objects in the order stated in the enabled update modes.

        :return: A list of GPOutputSentence objects in the order stated in the enabled update modes.
        """

    
        outputted_gp_sentences = [None for _ in self.enabled_update_modes]
        ...
        while received_uart_message:
            ...
            gp_sentence_obj: GPOutputSentence = self._message_id_to_outputsentence_type[
                message_id
            ].from_nmea_sentence(received_uart_message)

            if gp_sentence_obj:
                outputted_gp_sentences[
                    self.enabled_update_modes.index(
        ...
        return outputted_gp_sentences

    def receive_next_uart_msg(self) -> Optional[str]:
        """Reads the next NMEA sentence from the UART interface. Returns None if no message is available.

        :return: (Optional) The NMEA sentence string read from the UART interface.
        """
    ...
    def _send_update_modes_msg(self) -> None:
        ...
        full_set_mode_msg = f"PMTK314,{','.join(map(str, set_mode_data_list))}"
